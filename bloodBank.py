...
def create_tables_donor(mycursor): # function to create tables in database
    ...
    try:
        ...
        mycursor.execute(create_donors)
    except mysql.connector.Error as err:
...
def create_tables_recipients(mycursor): # function to create tables in database
    ...
    try:
        print("Creating table donors: ") # try create tables
        mycursor.execute(create_recipients)
    except mysql.connector.Error as err:
        if err.errno == errorcode.ER_TABLE_EXISTS_ERROR:
            print("already exists.") # error if the tables already exist
        else:
...
def insert_into_recipients(cursor):
    ...
    with open("recipients.csv", "r") as recipientsfile:
        recipientssfile = csv.reader(recipientsfile, delimiter=",")
...
try:
    mycursor.execute("USE {}".format(DATABASE_NAME)) # function to create database
except mysql.connector.Error as err:
    print("Database {} does not exist".format(DATABASE_NAME)) # if it does not exist
    if err.errno == errorcode.ER_BAD_DB_ERROR:
        creating_databases(mycursor, DATABASE_NAME)
        print("Database {} created succesfully.".format(DATABASE_NAME)) # if created
        cnx.database = DATABASE_NAME
        create_tables_donor(mycursor) # calling functions
        insert_into_donors(mycursor)
        create_tables_recipients(mycursor)
        insert_into_recipients(mycursor)
    else:
        print(err)
else:
